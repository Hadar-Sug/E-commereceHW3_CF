...
def main(q=4):
    if q == 4:
        ...
        K_list = [35]
        iterations = [20]
        checkpoints = np.arange(start=5, stop=100, step=5)
        logger = logging.getLogger(__name__)
        ...
        hyperparams_dict = {(iters, k): None for iters in iterations for k in K_list}
        # for reg in regularization:
        # iters, k = key[0], key[1]
        # part4.run_part_4_cross_validation(k=35, _lambda=reg, n_iters=20, splits=5, logger=logger)
